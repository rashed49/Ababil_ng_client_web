...
export const DETAILS_UI: string = "views/fixed-deposit-account/close?";
export const SUCCESS_MSG: string[] = ["fixed.deposit.account.close.success", "workflow.task.verify.send"];


@Component({
    selector: 'fixed-deposit-account-closing-form',
    templateUrl: 'fixed.deposit.account.closing.form.component.html'
})
export class FixedDepositAccountClosingFormComponent extends FormBaseComponent implements OnInit {
    fixedDepositAccount: FixedDepositAccount = new FixedDepositAccount();
    accountId: number;
    accountClosingForm: FormGroup;
    command: string = "FixedDepositAccountCloseCommand";
    ...
    constructor(private route: ActivatedRoute,
        ...
        this.accountNumberChanged
            ...
            .subscribe(model => {
                this.searchFixedDepositAccountByAccountNumber(model);
            });
    }
    ...
    accountCloseRequest: DemandDepositAccountCloseRequest;
    ngOnInit() {
            ...
            params => {
                ...
                this.subscribers.queryParamSub = this.route.queryParams.subscribe(
                    queryParams => {
                        ...
                        this.routeBack = queryParams['fixedDeposit'] ? queryParams['fixedDeposit'] : null;
    ...
    prepareAccountClosingForm(formData: DemandDepositAccountCloseRequest) {
        ...
        this.accountClosingForm.get('netPayable').valueChanges.subscribe(val => {
            ...
            this.amountInWords = this.amountInWordsService.convertAmountToWords(amount, this.fixedDepositAccount.currencyCode || null);
        });
        ...
        this.accountClosingForm.get('destinationAccountType').valueChanges.subscribe(
            value => {
                if (value) {
                    this.accountNumber = '';
                    this.selectedAccountForTransaction = new FixedDepositAccount();
    ...
    searchFixedDepositAccountByAccountNumber(accountNumber) {
        let accountQueryParams = new Map<string, string>().set('accountNumber', accountNumber);
        this.subscribers.fetchAccountDetailSub = this.fixedDepositAccountService.fetchFixedDepositAccounts(accountQueryParams).subscribe(
            account => {
    ...
    fetchFixedDepositAccount(accountId) {
        this.subscribers.fetchFixedDepositAccountDetailsSub = this.fixedDepositAccountService
            .fetchFixedDepositAccountDetails({ fixedDepositAccountId: accountId + "" }).subscribe(
                data => {
                    if (accountId === this.accountId) {
                        ...
                        this.accountClosingForm.get('profit').setValue(0);
                        this.accountClosingForm.get('accountId').setValue(this.accountId);
                        this.accountBalance = (this.fixedDepositAccount.fixedDepositAccountBalance.balance !== null) ? this.fixedDepositAccount.fixedDepositAccountBalance.balance : 0;
                        if (!this.taskId) {
                            this.accountClosingForm.get('profit').setValue(0); //this.fixedDepositAccount.fixedDepositAccountBalance.provisionalProfit
                            this.fetchCharges(0); //this.fixedDepositAccount.fixedDepositAccountBalance.balance
                        }
    ...
    onVerifierSelect(event: VerifierSelectionEvent) {
        ...
        // this.fixedDepositAccountService.demandDepositAccountClose(this.accountId, accountCloseRequest, urlSearchParams)
        //     .subscribe(data => {
    ...
    fetchCharges(amount) {
        ...
        urlQueryParamMap.set("activityId", 401);
