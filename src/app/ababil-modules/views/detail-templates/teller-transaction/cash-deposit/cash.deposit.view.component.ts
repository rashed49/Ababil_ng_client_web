...
import { CashReceive, IbtaInformation, BearerInfo, TellerDenominationView } from '../../../../../services/teller/domain/teller.domain.model';
...
@Component({
    selector: 'cash-deposit-view',
    templateUrl: './cash.deposit.view.component.html',
    styleUrls: ['./cash.deposit.view.component.css']
})

export class CashDepositViewComponent extends ViewsBaseComponent implements OnInit {
    ...
    cashDepositView: CashReceive = new CashReceive();
    ...
    ngOnInit() {
        ...
        if (this.voucherNumber) {
            this.subscribers.fetchCashDepositSub = this.tellerService.fetchCashRecieveDetail({ voucherNumber: this.voucherNumber }).subscribe(
                data => {
                    this.cashDepositView = data;
                    this.selectedCurrency = this.cashDepositView.transactionCurrencyCode;
                    this.amountCCYInWords = this.amountInWordService.convertAmountToWords(this.cashDepositView.amountCcy, this.selectedCurrency || null);
                    this.amountLCYInWords = this.amountInWordService.convertAmountToWords(this.cashDepositView.amountLcy, this.baseCurrency || null);

                    if (this.cashDepositView.transactionCreditAccountType === 'CASA' || this.cashDepositView.transactionCreditAccountType === 'SSP' || this.cashDepositView.transactionCreditAccountType === 'MTDR') {
                        this.cashDepositView.ibtaInformation = new IbtaInformation();
                        this.accountService.fetchAccountDetails({ 'accountId': this.cashDepositView.accountId }).subscribe(
                            data => {
                                ...
                                this.selectedSubAccountType = this.cashDepositView.transactionCreditAccountType;
                                this.fetchAccountDetails();
                            }
                        )
                    }
                    else if (this.cashDepositView.transactionCreditAccountType == 'GL') {
                        this.glAccountService.fetchGlAccountDetails({ 'id': this.cashDepositView.accountId }).subscribe(
                            data => {
                    ...
                    else if (this.cashDepositView.transactionCreditAccountType == 'SUBGL') {
                        this.glAccountService.fetchSubGlDetails({ 'subsidiaryLedgerId': this.cashDepositView.accountId }).subscribe(
                            data => {
                                this.glTxn = false;
                                this.accountTxn = false;
                                this.subglTxn = true;
                    ...
                    this.subscribers.fetchSub = this.tellerService.fetchTellerById({ id: this.tellerId }).subscribe(
                        tellerDetail => {
                            ...
                            this.tellerLimit = this.teller.tellerLimits.filter(limit => limit.currencyCode === this.cashDepositView.transactionCurrencyCode)[0] ? this.teller.tellerLimits.filter(limit => limit.currencyCode === this.cashDepositView.transactionCurrencyCode)[0] : new TellerLimit();
                            if (this.cashDepositView.transactionCurrencyCode) {
                            }
                            if (this.teller.denominationRequired) {
                                this.denominationChecker = false;
                                this.cashDepositView.tellerDenominationTransactions.map(
                                    data => {
                                        if (data) {
                    ...
                    this.subscribers.fetchBal = this.tellerService.fetchBalanceByTellerIdAndCurrencyCode({
                        tellerId: this.tellerId, currencyCode: this.cashDepositView.transactionCurrencyCode
                    }).subscribe(
                        tellerBalance => {
    ...
    postTransaction() {
        this.tellerService.postUnpostedTransactions({ voucherNumber: this.voucherNumber }).subscribe({
            complete: () => {
                this.notificationService.sendSuccessMsg('unposted.transaction.posted.success');
        ...
        this.subscribers.glAccountFetchSub = this.glAccountService.
            ...
            .subscribe(data => {
                ...
                if (this.glAccount.isIbtaRequired) {
                    ...
                    if (this.cashDepositView.ibtaInformation.originatingBranchId) {
                        this.fetchBranches(this.cashDepositView.ibtaInformation.originatingBranchId, (name) => this.originatingBranch = name);
                    }
                    if (this.cashDepositView.ibtaInformation.respondingBranchId) {
                        this.fetchBranches(this.cashDepositView.ibtaInformation.respondingBranchId, (name) => this.respondingBranch = name);
                    }
                }
                else {
    ...
    fetchTellerDetail(tellerId: number) {
        this.subscribers.fetchSub = this.tellerService.fetchTellerById({ id: tellerId }).subscribe(
            tellerDetail => {
                this.teller = tellerDetail;
                this.fetchTellerBalance(tellerId, this.cashDepositView.transactionCurrencyCode);
                this.tellerLimit = this.teller.tellerLimits.filter(limit => limit.currencyCode === this.cashDepositView.transactionCurrencyCode)[0] ? this.teller.tellerLimits.filter(limit => limit.currencyCode === this.cashDepositView.transactionCurrencyCode)[0] : new TellerLimit();
            });
    }
    ...
    fetchIbtaTxnParticulars() {
        this.tellerService.fetchIbtaTxnParticularsDetails({ 'id': this.cashDepositView.ibtaInformation.ibtaTrCode }).subscribe(
            data => {
