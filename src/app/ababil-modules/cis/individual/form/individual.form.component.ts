...
import { mapper } from './individual.info.mapper';
...
export class IndividualFormComponent extends BaseComponent implements OnInit, OnChanges {
    imageApiUrl: string;
    uuid: string;
    individualId: number;
    ...
    prepareIndividualForm(formData: IndividualInformation) {
        ...
        this.subscribers.fetchCustomerSub = this.cisService.fetchCustomer({ id: this.customerId }).subscribe(
            customer => {
                let customerName = customer.name.split(" ");
                this.individualForm.controls.firstName.setValue(formData.firstName ? formData.firstName : customerName[0]);
                this.individualForm.controls.middleName.setValue(formData.middleName ? formData.middleName : customerName[1] ? customerName[1] : '');
                this.individualForm.controls.lastName.setValue(formData.lastName ? formData.lastName : customerName[2] ? customerName.splice(2, customerName.length).join(' ') : '');
            }
        )
        ...
        this.individualForm = this.formBuilder.group({
            firstName: [formData.firstName ? formData.firstName : this.firstName, [Validators.required, abbabilValidators.personNameValidator]],
            middleName: [formData.middleName ? formData.middleName : this.middleName, [abbabilValidators.personNameValidator]],
            lastName: [formData.lastName, [abbabilValidators.personNameValidator]],
            fathersFirstName: [formData.fathersFirstName, [Validators.required, abbabilValidators.personNameValidator]],
            fathersMiddleName: [formData.fathersMiddleName, [abbabilValidators.personNameValidator]],
            fathersLastName: [formData.fathersLastName, [abbabilValidators.personNameValidator]],
            mothersFirstName: [formData.mothersFirstName, [Validators.required, abbabilValidators.personNameValidator]],
            mothersMiddleName: [formData.mothersMiddleName, [abbabilValidators.personNameValidator]],
            mothersLastName: [formData.mothersLastName, [abbabilValidators.personNameValidator]],
            maritalStatus: [formData.maritalStatus, [Validators.required]],
            spouseName: [formData.spouseName, [abbabilValidators.personNameValidator]],
            ...
            mobileNumber: [formData.contactInformation.mobileNumber, [abbabilValidators.phoneNumberValidator]],
            ...
            otherBankName: [null, [abbabilValidators.personNameValidator]],
            otherBankBranchName: [null, [abbabilValidators.personNameValidator]],
            otherBankAccountTypes: null,
            creditCardNumber: null,
            creditCardBankName: [null, [abbabilValidators.personNameValidator]],
            ...
            permanentAddressAsPresentChecker: null,
            profAddressAsPermanentChecker: null,
            profAddressAsPresentChecker: null,
            sourceOfFunds: null,
        });
        ...
        this.subscribers.fetchBaseCountryIdSub = this.cisMiscellaneousService.getConfiguration({ key: 'BASE_COUNTRY' }).subscribe(
            data => {
                ...
                if (this.individualForm.get('birthCountryId').value == this.baseCountryId) {
                    ...
                    this.individualForm.controls.birthPlaceId.setValidators(Validators.required);
                    this.individualForm.controls.birthPlaceId.updateValueAndValidity();
                    // this.individualForm.removeControl('foreignBirthPlace');
                    this.individualForm.controls.foreignBirthPlace.clearValidators();
                    this.individualForm.controls.foreignBirthPlace.updateValueAndValidity();
                }
                else {
                    this.nrbBirthPlace = true;
                    ...
                    this.individualForm.controls.foreignBirthPlace.setValidators(Validators.required);
                    this.individualForm.controls.foreignBirthPlace.updateValueAndValidity();
                    this.individualForm.controls.birthPlaceId.clearValidators();
                    this.individualForm.controls.birthPlaceId.updateValueAndValidity();
                }
            }
        )
        ...
        this.individualForm.get('permanentAddressAsPresentChecker').valueChanges.subscribe(
            permanentAddressAsPresentChecker => {
                if (permanentAddressAsPresentChecker) {
                    this.permanentAddressComponent.viewMode = true;
                    this.permanentAddress = this.presentAddressComponent.extractData();
                    console.log(this.presentAddressComponent.extractData());

                }
                else {
                    this.permanentAddress = new Address();
                    this.permanentAddressComponent.viewMode = false;
                }
            }
        )


        this.individualForm.get('profAddressAsPermanentChecker').valueChanges.subscribe(
            profAddressAsPermanentChecker => {
                if (profAddressAsPermanentChecker) {
                    this.professionalAddress = this.permanentAddressComponent.extractData();
                    this.presentAddressCheckerStatus = true;
                    this.professionalAddressComponent.viewMode = true;
                    console.log(this.permanentAddressComponent.extractData());
                    ...
                    this.professionalAddressComponent.viewMode = false;
        ...
        this.individualForm.get('profAddressAsPresentChecker').valueChanges.subscribe(
            profAddressAsPresentChecker => {
                if (profAddressAsPresentChecker) {
    ...
    formInvalid() {
        ...
        return (this.individualForm.invalid || this.presentAddressComponent.isInvalid() || this.permanentAddressComponent.isInvalid() || this.selectedSourceOfFunds.length < 1 || this.occupationComponent.addedOccupations.length < 1);
    }

    formValid() {
        if (!this.individualForm) return false;
        return (this.individualForm.valid && this.presentAddressComponent.isValid() && this.permanentAddressComponent.isValid() && this.selectedSourceOfFunds.length > 0 || this.occupationComponent.addedOccupations.length > 0);
    }
    ...
    setImage(event) {
        let imageBody = event;
        let imageBlob = this.makeblob(event);
        this.subscribers.saveSub = this.imageUploadService.uploadIndividualPicture({ id: this.individualId + '', uuid: this.uuid }, imageBlob).subscribe(
            (data) => {
                this.notificationService.sendSuccessMsg("image.upload.success");
                this.individualImageComponent.nativeElement.src = this.imageApiUrl + "/" + this.individualId + "/image";
            });
    }
    markAsTouched() {
    ...
    onPresentAddressChange(event) {
        if (this.individualForm.get('permanentAddressAsPresentChecker').value && this.permanentAddress !== event) {
            this.permanentAddress = event;
        }
        if (this.individualForm.get('profAddressAsPresentChecker').value && this.professionalAddress !== event) {
            this.professionalAddress = event;
        }
    }
    onPermanentAddressChange(event) {
        if (this.individualForm.get('profAddressAsPermanentChecker').value && this.professionalAddress !== event) {
            this.professionalAddress = event;
        }
    }
}