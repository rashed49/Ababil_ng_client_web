...
@Component({
  selector: 'ababil-cis',
  templateUrl: './cis.inactive.customer.component.html',
})
export class CISInactiveCustomerComponent extends BaseComponent implements OnInit {
  msgs: Message[] = [];
  customer: Customer;
  customers: any;
  ...
  loadCustomerLazily(event: LazyLoadEvent) {
    ...
    this.urlSearchMap.set("active", "false");
  ...
  onRowSelect(event) {
    switch (event.data.customerType) {
      case 'INDIVIDUAL_SINGLE':
        this.cisService.fetchSubjects({ id: event.data.id + '' }).subscribe(
          data => {
            if (data.length > 0) {
              this.individualSubjectId = data[0].id;
            }
      ...
      case 'INDIVIDUAL_JOINT':
        this.cisService.fetchSubjects({ id: event.data.id + '' }).subscribe(
          data => {
            if (data.length > 0) {
              this._router.navigate(['/customer/joint/form', event.data.id]);
            } else {
              this._router.navigate(['/customer/subject-individual'], {
                queryParams: { customerType: 'JOINT', id: null, customerId: event.data.id, customerName: event.data.name }
              });
            }
          });
        break;
      ...
      case 'ORGANIZATION':
        this.cisService.fetchSubjects({ id: event.data.id + '' }).subscribe(
          data => {
            if (data.length > 0) {
              this.organizationSubjectId = data[0].id;
            }
            ...
            this._router.navigate(['customer/organization-details'],
              {
                queryParams: {
                  ...
                  applicantId: this.organizationSubjectId,
                  customerId: event.data.id,
                  customerName: event.data.name
                },
                queryParamsHandling: 'merge'
    ...
    this.subscribers.fetchCustomerSub = this.cisService.fetchCustomer({ id: cusId }).subscribe(
      data => {
        if (cusType === 'INDIVIDUAL_SINGLE') {
          ...
          this._router.navigate(['/customer/subject-individual'], {
            queryParams: { customerType: 'JOINT', id: null, customerId: data.id, customerName: data.name }
          });
